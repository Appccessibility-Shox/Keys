...
$("html").on("keydown", async (e) => {
    if (
        ...
        imageLikeTargets = targets.filter((t) => {
            return t.querySelector("img, svg, i") || t.classList.contains("Keys-Should-Generate-Floating-Text") || (t.nodeName == "INPUT" && t.type != "text" && t.type != "search") || containsOrIsImage(t)
        })
        ...
        imageLikeTargets.filter((t) => {
            return !elementIsHiddenViaStyles(t)
        }).forEach((target) => {
            keyString = combos.find(isAbsent) // TODO: Optimize
            modelMember = {key: keyString, element: target, type: ElementTypes.imagelike, responsibleNode: target.querySelector("img, svg, i, .Keys-isEssentiallyAnImage") || target }
            model.push(modelMember)
        })
...
click =(element, shouldOpenInBackgroundTab)=> {
    ...
    if (shouldOpenInBackgroundTab && url) {
        browser.runtime.sendMessage({message: "metaOpen", options: {"url": url} })
    } else {
        element.click()
    }
}
...
elementHasBackgroundImage =(el)=> {
    return window.getComputedStyle(el).getPropertyValue('background-image') !== 'none';
}
